...
if __name__ == '__main__':
    ...
    for i in range(10, 100, batch_size):
        ...
        tstart = time.time()
        tweets_df = DataAccess.sample_control(
            lower=i / 100, upper=(i + batch_size) / 100)
        ...
        print("[%.2f/%.2f] Sampled %d tweets in %.2f secs" %
              (i / 100, (i + batch_size) / 100, len(tweets_df), tend - tstart))

        tstart = time.time()

        for clf, model_name in ensemble_models:
            column_name = model_name + '_predict'
            tweets_df[column_name] = clf.predict_proba(
                tweets_df.clean_text)[:, 1]

        tend = time.time()

        print(
            "Completed %d predictions in %.3f secs" %
            (len(tweets_df), tend - tstart))
        ...
        outname = './data/test-sampled_ensemble_proba_%d-%dperc.csv' % (
            i, i + batch_size)
